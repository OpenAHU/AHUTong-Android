...
import com.ahu.ahutong.feature.widget.R
    ...
    private fun updateAppWidget(
        ...
        val schedule = when (
            val result = runCatching {
                runBlocking { EntryPointAccessors.fromApplication(AppContextHolder.requireApp(), ScheduleEntryPoint::class.java).scheduleRepository().getSchedule(false) }
            }.getOrNull()
        ) {
        ...
        val remainingCourses = todayCourses.filter {
            currentMinutes <= CourseTimetable.getCourseTimeRangeInMinutes(it).last
        }
        ...
        if (displayCourses.isEmpty()) {
            ...
            displayCourses.forEach {
                val currentRange = CourseTimetable.getCourseTimeRangeInMinutes(it)
